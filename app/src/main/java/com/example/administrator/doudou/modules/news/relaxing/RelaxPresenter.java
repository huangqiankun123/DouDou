...
public class RelaxPresenter extends BasePresenter implements NewImp.Presenter {

    private NewImp.View mNewsListFrag;

    public RelaxPresenter(NewImp.View mNewsListFrag) {
    ...
    public void refreshNews() {
        ...
        currentIndex =0;
        ApiManager.getInstence().getTopNewsServie().getDadaNews(currentIndex+"")
                .map(new Function<RelaxNewsList, ArrayList<NewsBean>>() {
                    @Override
                    public ArrayList<NewsBean> apply(RelaxNewsList relaxNewsList) throws Exception {
                        return relaxNewsList.getDadaNewsArrayList();
                    }
    ...
    public void loadCache() {
        DiskCacheManager manager = new DiskCacheManager(MyApplication.getContext(),Constants.CACHE_NEWS_FILE);
        ArrayList<NewsBean> topNews= manager.getSerializable(Constants.CACHE_CARTOON_NEWS);
        if (topNews!=null){
