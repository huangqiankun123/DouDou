...
public class SportsPresenter extends BasePresenter implements NewImp.Presenter {

    private NewImp.View mNewsListFrag;

    public SportsPresenter(NewImp.View mNewsListFrag) {
    ...
    public void refreshNews() {
        ...
        currentIndex =0;
        ApiManager.getInstence().getTopNewsServie().getSportNews(currentIndex+"")
                .map(new Function<SportNewsList, ArrayList<NewsBean>>() {
                    @Override
                    public ArrayList<NewsBean> apply(SportNewsList sportNewsList) throws Exception {
    ...
    public void loadCache() {
        DiskCacheManager manager = new DiskCacheManager(MyApplication.getContext(),Constants.CACHE_NEWS_FILE);
        ArrayList<NewsBean> topNews= manager.getSerializable(Constants.CACHE_SPORT_NEWS);
        if (topNews!=null){
